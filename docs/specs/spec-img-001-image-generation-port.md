...
| Derived From | ADR-010 (incl. **A2**) · `POLICY_RUNTIME_DEPLOYMENT_LAYER_SPEC` §6 |
| Supersedes | SPEC-IMG-001 v0.2 (Contract Freeze without Creator ⊥ Reader surface posture) |
...
| Spike Implementation Authorization | **Granted** → allowlists in SPIKE-IMG-001 and SPIKE-IMG-002 only |
| Production Authorization | **Not granted** |

Spike Authorization does **not** imply Production Authorization.
Contract Freeze does **not** authorize production Admin / Rollout / Reader wiring.
...
This SPEC freezes **contracts, surface posture knobs, and policy knobs**. It does not freeze providers, models, pricing, showcase works, or `Creator = Local`.
...
5. **IMG-INV-05 — No Creator=Local freeze.** Architecture / Runtime Contract MUST NOT freeze `Creator Runtime = Local` (or any vendor). Local / Cloud bindings are Deployment.
...
* Creator Runtime **MAY** use Local as a Deployment provider when deployment evidence supports it; Cloud (or other hosted) fallback remains a Deployment concern.
* Reader Runtime **MUST NOT** treat an author-laptop Local endpoint as a production availability dependency. If Reader ever invokes generation, binding MUST be hosted/cloud-class Deployment (requires separate Production Authorization for that scope).
...
Port shape is unchanged by ADR-010 **A2**; A2 constrains *who may bind which Deployment*, not the Port fields.
...
* Surface-as-provider freezes (e.g. requiring Creator→Local)
...
* Local Creator pipelines (SPIKE-IMG-002) — Deployment candidate only
...
* Reader hot-path generation (Deployment may forbid; Production Authorization still required if ever enabled)
...
**Denylist:** production avatar action, Rollout, Discovery, production Copilot, Reader runtime, DB schema, production Cloudinary contracts, any Runtime Truth path outside the allowlist, elevating Local to Production/Creator Deployment Default without a separate grant.

**Exit Criteria & Findings:** defined in each Spike document. Passing a Spike does **not** auto-grant Production Authorization or Creator-Local Deployment Default.

---

## 10. Refs

* ADR-010 (v1.2 · Amendments **A1**, **A2**)
...
* `docs/deployment/deployment-defaults.md` (per-surface Deployment recommendations)
