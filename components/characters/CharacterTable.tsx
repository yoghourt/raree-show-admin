...
export function CharacterTable({
  ...
  const selectedNames = React.useMemo(() => {
    ...
    return characters
      .filter((c) => idSet.has(c.tsid))
      .map((c) => c.name)
      .slice(0, 8);
  }, [characters, deleteDialog]);
          ...
          {loading ? (
            ...
            <>
              ...
              <Table className="min-w-max">
                ...
                <TableBody>
                  {characters.map((c) => {
                    const isChecked = selected.has(c.tsid);
                    return (
                      <TableRow
                        key={c.tsid}
